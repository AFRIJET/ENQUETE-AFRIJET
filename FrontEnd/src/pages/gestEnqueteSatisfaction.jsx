...
const gestEnqueteSatisfaction = () => {
  ...
  const handleDownloadReportExcel = () => {
    ...
    axios.get(`${apiUrl}/admin/generate_excel_report`, {
      params: { enquete: "Enquete_Satisfaction", StartDate: startDate, EndDate: endDate },
      ...
      .then((response) => {
        ...
        if (response.status === 200 && response.data) {
          ...
          link.href = url;
          link.setAttribute('download', 'rapport_enquete_satisfaction.xlsx');
    ...
    axios.get(`${apiUrl}/admin/generate_csv_report`, {
      ...
      .then(response => {
        ...
        if (response.status === 200 && response.data) {
          ...
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', 'rapport_enquete_satisfaction.csv');
          document.body.appendChild(link);
          link.click();
          link.remove();
          setIsModalOpen((prev) => !prev);
  ...
  return (
    <div>
      ...
      <div className=''>
        <p className='text-xl flex items-center justify-center text-black'>No data found</p>
      </div>
    </div>
  )
}

export default gestEnqueteSatisfaction